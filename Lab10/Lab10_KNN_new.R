...
setwd("~/Documents/бэкап с офисного ПК_xeon/R_projects/Teaching_R/lection_3")
...
# в папке вы найдете архивный файл iris_data-set.zip
# он содержит таблицу с биологическими признаками цветов ириса разного вида
# вид цветка определяется сочетанием его характеристик. Этот дата-сет подходит для решения задачи классификации 
# методом KNN

# ваша задача - решить задачу классификации методом KNN. Визуализируйте данные.
# Код должен отправлять свои результаты на указанную в КОНСОЛИ эл почту пользователя
# Как обычно, код должен быть сохранен в отдельном репозитории Вашего Гитхаб аккаунта.
# Результат работы - ссылка на репозиторий, которую нужно отправить мне в Тимс

# В качестве необязательного пункта задания - попробуйте решение этой задачи 2мя способами:
# - KNN
# - множественной регрессией (см Лабораторную работу №9)
