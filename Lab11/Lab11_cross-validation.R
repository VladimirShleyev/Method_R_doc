...
#Суть практического задания: 1) скачать файлы Lab11, iris.csv. 
# 2) так модифицировать код лабораторной (можете дописать нужные строки/отредактировать), 
# чтобы: прочесть в память дата-сет, применить к нему сначала просто случайное разделение выборки 
# на трейн/тест, а затем - к-кратную повторную выборку 3) записать получившееся разделение предикторов 
# и переменной-отклика в отдельные csv-файлы

# Должно получиться 4 csv файла - 2 после применения случайного разделения, 
# 2 - после применения к-кратной повторной выборки

# Эти 4 файла - выкладывайте в чат Teams

# Используйте ф-ии read.csv() и write.csv()
